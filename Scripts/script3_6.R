...
#Lectura de datos
datos = read.csv("datos3_6.csv", sep = ";")
...
resi4 = rstandard(modelo)
ad.test(resi4)

  #Grafico:
g1 = ggplot(modelo, aes(qqnorm(resi4)[[1]], .resid)) +
...
bartlett.test(resi4,Tratamientos)
...
#Gráfico:
g5 = as.data.frame(resi4) %>%
  ggplot(aes(1:length(resi4), y=resi4)) +
  geom_point(size = 3.5) +
  geom_line() +
...
#Pregunta d
  ...
  #-Prueba de Duncan
duncan.test(modelo, "Tratamiento",console=TRUE, group=FALSE)

#Pregunta e

(tc = ((30.17-18.03)-11)/sqrt(2*0.37/4))
